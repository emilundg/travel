<template>
  <div id="app">
    ...
    <b-container v-if="tab === 0" class="container-fluid landing-page" fluid>
      <div class="row mt-5">
        <b-card class="mt-5 col-12 col-sm-8 col-md-6 col-lg-4">
          ...
          <GmapMap @click="testFunc" :center="{lat:currentLocation.lat, lng:currentLocation.lng}" :zoom="16" map-type-id="roadmap" style="width: 500px; height: 300px">
    ...
    methods: {
      ...
      geolocation: function() {
        navigator.geolocation.getCurrentPosition((position) => {
          ...
          this.markers[0].position.lat = this.currentLocation.lat;
          this.markers[0].position.lng = this.currentLocation.lng;
        });
      },
      onSubmit(e) {
        this.loading = true;
      ...
      async submitHotel() {
        await this.$binding("act", Firebase.firestore().collection(this.hotelInput.toLowerCase()))
          .then((act) => {
            if (act.length === 0) {
              this.errorMessage = 'No hotel activities found!'
            } else {
              this.activities = act
              this.hotel = this.hotelInput.toUpperCase()
              this.changeTab(1);
            }
          })
      },
